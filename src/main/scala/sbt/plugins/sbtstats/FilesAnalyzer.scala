...
class FilesAnalyzer extends Analyzer {
  def analyze(sources: Seq[File], packageBin: File, encoding: String) = {
    ...
    val avgSize = if (sources.isEmpty) 0 else totalSize / sources.length
    val avgLines =
      if (sources.isEmpty) 0
      else sources.map(s => Source.fromFile(s, encoding).getLines.length).sum / sources.length
